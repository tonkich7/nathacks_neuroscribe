...
#FILE_PATH = './experiment_data/direction_features_labels_'
FILE_PATH = './experiment_data/direction_epochs_'
...
def one_hot_encode(y):
    labels = np.unique(y)
    y_indices = (y - 1).astype(int) #maps 1,2,3,4 to 0,1,2,3
...
def main():
    ...
    #     loo_acc = leave_one_out(X, one_hot_y, epochs.info, alpha=1e-3, num_epochs=3, batch_size=5, regularization=0.0001)
    #     print(loo_acc)
    #     total_acc += loo_acc
    ...
    model, scaler, vectorizer, num_samples = train_final_model(X, one_hot_y, epochs.info, alpha=1e-3, num_epochs=5, batch_size=5, regularization=0.001)
    print("Num_samples:", num_samples)
    print("Num_samples per channel:", num_samples//4)
    with open("./direction_model", 'wb') as file:
